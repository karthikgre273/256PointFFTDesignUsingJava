...
public class DmemGenerator_withparallelButterflies_temp {
	double pi=3.14;
	//256 bits
	//	int Npoint=256;
	...
	public static void main(String[] args) throws ParseException, IOException {
		DmemGenerator_withparallelButterflies_temp obj=new DmemGenerator_withparallelButterflies_temp();
	...
	public void caseStatementGenerator() throws IOException{
		...
		for (int stage = 0; stage < numberOfStages; stage++) {
			...
			for(int p=0; p<Npoint/Math.pow ( 2, (stage+1));p++){
				int firstVariable=(int) ((Math.pow ( 2, (stage+1)))*p);
				for(int q=0;q<(int)Math.pow ( 2, (stage));q++){		
					int input1=firstVariable+q;
					int input2=firstVariable+q+(int)Math.pow ( 2, (stage));		
					printCaseStatements(writer,input1,input2,stage,q);	
	...
	private void printCaseStatements(BufferedWriter writer, int input1, int input2, int currentStage, int q) throws IOException {
		...
		//writer.write("\t\t\t\t pushinB"+count%4+" <= 1'b1;\n");
			writer.write("\t\t\t\t imgIp1B"+count%4+" <= imagWorkingBuffer["+input1+"];\n");
			writer.write("\t\t\t\t realIp1B"+count%4+" <= realWorkingBuffer["+input1+"];\n");
			writer.write("\t\t\t\t imgIp2B"+count%4+" <= imagWorkingBuffer["+input2+"];\n");
			writer.write("\t\t\t\t realIp2B"+count%4+" <= realWorkingBuffer["+input2+"];\n");
		
		int real=twiddleGenerator.generate(false,currentStage,q);
		int imag=twiddleGenerator.generate(true,currentStage,q);


		writer.write("\t\t\t\t twiddleFactorRealB"+count%4+" <="+real+";\n");

		writer.write("\t\t\t\t twiddleFactorimgB"+count%4+" <="+imag+";\n");



		writer.write("\t\t\t\t stateIp1B"+count%4+"<="+(numberOfBitsToaddress)+"'d" +input1+";\n");
		writer.write("\t\t\t\t stateIp2B"+count%4+"<="+(numberOfBitsToaddress)+"'d" +input2+";\n");
		if(count%4 ==3)
			writer.write("\t\t\t end\n");
		count++;
	}
	private void printDefaultCaseStatements(BufferedWriter writer) throws IOException {
		...
		for(int i=0;i<4;i++){
			...
			writer.write("\t\t\t\t imgIp1B"+i%4+" <= "+(noOfBitsUsedForRepresentingEachNumber)+"'d0;\n");
			writer.write("\t\t\t\t realIp1B"+i%4+" <= "+(noOfBitsUsedForRepresentingEachNumber)+"'d0;\n");
			writer.write("\t\t\t\t imgIp2B"+i%4+" <= "+(noOfBitsUsedForRepresentingEachNumber)+"'d0;\n");
			...
			writer.write("\t\t\t\t twiddleFactorimgB"+i%4+" <="+(noOfBitsUsedForRepresentingEachNumber)+"'d0;\n");
...
//		writer.write("\t\t\t default: begin\n");
//		for(int i=0;i<4;i++){		
//			writer.write("\t\t\t\t imgIp1B"+i%4+" <= "+(noOfBitsUsedForRepresentingEachNumber)+"'dx;\n");
//			writer.write("\t\t\t\t realIp1B"+i%4+" <= "+(noOfBitsUsedForRepresentingEachNumber)+"'dx;\n");
//			writer.write("\t\t\t\t imgIp2B"+i%4+" <= "+(noOfBitsUsedForRepresentingEachNumber)+"'dx;\n");
...
//			writer.write("\t\t\t\t twiddleFactorimgB"+i%4+" <="+(noOfBitsUsedForRepresentingEachNumber)+"'dx;\n");
...
//		writer.write("\t\t\t end\n");
	...
	private void printHardCodeStatements(BufferedWriter writer) throws IOException {
		...
		//		writer.write("module fftCalculator(input clock,reset,\ninput[19:0] realInput,complexInput, input startIn,\n"
		//				+ "output reg outputFlag,\noutput reg [19:0] realOutput,complexOutput);");
		//
