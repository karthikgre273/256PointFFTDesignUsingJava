...
public class DmemGenerator_withparallelButterflies {
	...
	public static void main(String[] args) throws ParseException, IOException {
		DmemGenerator_withparallelButterflies obj=new DmemGenerator_withparallelButterflies();
	...
	public void caseStatementGenerator() throws IOException{
		...
		for (int currentStage = 0; currentStage < numberOfStages; currentStage++) {
			if(currentStage==0) {stage0flag=true;}
	...
	private void printCaseStatements(BufferedWriter writer, int input1, int input2, int currentStage, int q) throws IOException {
		...
		if(stage0flag){
			writer.write("\t\t\t\t complexIp1B"+count%4+" = complxMem["+reverseBits(input1)+"];\n");
			writer.write("\t\t\t\t realIp1B"+count%4+" = realMem["+reverseBits(input1)+"];\n");
			writer.write("\t\t\t\t complexIp2B"+count%4+" = complxMem["+reverseBits(input2)+"];\n");
			writer.write("\t\t\t\t realIp2B"+count%4+" = realMem["+reverseBits(input2)+"];\n");
		}
		else{
		...
		int real=twiddleGenerator.generate(false,stage0flag?0:currentStage,q);
		int imag=twiddleGenerator.generate(true,stage0flag?0:currentStage,q);
		...
		writer.write("\t\t\t\t stateIp1B"+count%4+"<="+(numberOfBitsToaddress)+"'d" +(AddressGenerator(stage0flag?0:currentStage,input1)+Npoint)+";\n");
		writer.write("\t\t\t\t stateIp2B"+count%4+"<="+(numberOfBitsToaddress)+"'d" +(AddressGenerator(stage0flag?0:currentStage,input2)+Npoint)+";\n");
